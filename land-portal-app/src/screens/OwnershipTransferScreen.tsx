...
export default function OwnershipTransferScreen() {
    ...
    const handleSubmit = () => {
        ...
        const referenceNumber = `TRF-${new Date().getFullYear()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
        const taxInfo = calculateTax();

        setTimeout(() => {
            Alert.alert(
                'Success',
                `Ownership transfer request submitted!\nReference: ${referenceNumber}\nTransfer Price: ${parseFloat(formData.transferDetails.transferPrice || '0').toLocaleString()} ETB\nTax (${taxInfo.taxRate}%): ${parseFloat(taxInfo.taxAmount).toLocaleString()} ETB\nTotal: ${parseFloat(taxInfo.totalAmount).toLocaleString()} ETB\nEstimated time: 14-30 business days`,
                [
                    {
                        text: 'Track Request',
            ...
            >
                <View className="flex-1">
                    ...
                    >
                        ...
                        >
                            {(sections || []).map((section: any, index: number) => renderSectionCard(section, index))}
                        </ScrollView>
                    </LinearGradient>
                </View>
            </KeyboardAvoidingView>
