...
type RootStackParamList = {
    OTPVerification: { phone?: string; demoCode?: string };
    Login: undefined;
};
...
export default function OTPVerificationScreen() {
    ...
    const demoCode = route.params?.demoCode;
    ...
    const handleVerify = async () => {
        ...
        if (!phone) {
            Alert.alert('Error', 'Phone number missing. Please register again.');
            return;
        }
        ...
        <View className="flex-1">
            ...
            >
                ...
                <View className="items-center mb-10">
                    ...
                    {phone ? (
                        <Text className="text-white font-semibold text-center text-base mt-1">{phone}</Text>
                    ) : null}
                    {demoCode ? (
                        <Text className="text-white/90 text-center text-sm mt-3 bg-white/10 px-4 py-2 rounded-lg">
                            Dev code: {demoCode}
                        </Text>
                    ) : null}
                </View>
